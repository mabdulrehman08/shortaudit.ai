    ...
    <section className="relative overflow-hidden px-6 pb-16 pt-8 sm:px-8 lg:px-10">
      <div className="mx-auto flex max-w-7xl items-center justify-between rounded-full border border-white/10 bg-white/[0.03] px-4 py-3 backdrop-blur">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-white to-zinc-400 text-black shadow-glow">
            <Sparkles size={20} />
          </div>
          <span className="text-sm font-bold tracking-tight text-white">ShortAudit AI</span>
        </div>
        <a href="#dashboard" className="hidden rounded-full bg-white px-4 py-2 text-sm font-bold text-black transition hover:bg-zinc-200 sm:block">
          Analyze My Video
        </a>
      </div>
      ...
      <div className="mx-auto grid max-w-7xl gap-12 py-20 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
        <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          ...
          <div className="mt-9 flex flex-col gap-4 sm:flex-row">
            <a href="#dashboard" className="group inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-white to-zinc-400 px-7 py-4 font-bold text-black shadow-glow transition hover:scale-[1.02]">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.15 }} className="glass gradient-border rounded-[2rem] p-5 shadow-card">
          <div className="rounded-[1.5rem] border border-white/10 bg-black/80 p-5">
            <div className="mb-8 flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.28em] text-zinc-300">Live Audit</p>
                <h3 className="mt-2 text-2xl font-bold text-white">Distribution risk</h3>
              </div>
              <div className="rounded-2xl bg-white/10 px-3 py-2 text-sm font-bold text-white">82% confidence</div>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              {[
                ['Hook', '7.8/10', Sparkles, 'from-white to-zinc-500'],
                ['Shadowban', '24%', Zap, 'from-zinc-200 to-zinc-600'],
                ['Retention', 'Medium', BarChart3, 'from-white to-zinc-700'],
                ['Viral score', '78', ArrowRight, 'from-zinc-100 to-zinc-500'],
              ].map(([label, value, Icon, gradient]) => (
                <div key={label} className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
                  <div className={`mb-5 flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br ${gradient} text-black`}>
                    <Icon size={19} />
                  </div>
